        ...
        <div className="lg:w-[47%] w-full md:border-r md:border-[#1E2D3D] sm:w-full flex items-center h-[100vh]">
          {
            showSucessMessage ? (
              <form action="#" className="space-y-8 w-full py-5 md:h-[100vh] px-4" method="POST" onSubmit={handleSubmit(onSubmit)}>
                ...
                <div>
                  ...
                  <input type="text" name='name'
                    ...
                    onChange={handleInputChange} value={formData.name} id="name" className="shadow-sm bg-[#011221] border border-[#1E2D3D] text-[#607B96] text-sm rounded-lg  block w-full p-2.5" placeholder="name" />
                  {errors.name && <small className='text-red-700'>*Name is required*</small>}
                </div>
                <div>
                  <label htmlFor="subject" className="block mb-2 text-sm font-medium text-[#607B96] dark:text-gray-300">email</label>
                  <input type="email" name='email'
                    {...register("email", { required: true })}
                    onChange={handleInputChange} value={formData.email} id="email" className=" bg-[#011221] border border-[#1E2D3D] text-[#607B96] text-sm rounded-lg  block w-full p-2.5" placeholder="abc@gmail.com" />
                  {errors.email && <small className='text-red-700'>*Email is required*</small>}
                </div>
                <div className="sm:col-span-2">
                  <label htmlFor="message" className="block mb-2 text-sm font-medium text-[#607B96] dark:text-gray-400">Your message</label>
                  <textarea id="message"
                    {...register('message', { required: true })}
                    onChange={handleInputChange}
                    value={formData.message} name="message" rows="6" className="block p-2.5  bg-[#011221] border border-[#1E2D3D] w-full text-sm text-[#607B96] rounded-lg shadow-sm focus:ring-primary-500 focus:border-primary-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" placeholder="Leave a comment..."></textarea>
                  {errors.message && <small className='text-red-700'>"Please write the message"</small>}
                </div>
                <button type="submit" className="py-3 bg-[#1E2D3D] border border-[#1E2D3D] px-5 text-sm font-medium text-center text-white rounded-lg bg-primary-700 sm:w-fit hover:bg-primary-800 focus:ring-4 focus:outline-none focus:ring-primary-300 dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800">Send message</button>
              </form>
            ) : (
              <div className="text-center w-72 mx-auto">
                ...
                <a href='/ContactPage' type="submit" className="py-3 bg-[#1E2D3D] border border-[#1E2D3D] px-5 text-sm font-medium text-center text-white rounded-lg  sm:w-fit  focus:ring-4 focus:outline-none focus:ring-primary-300 dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800">Send-new-message</a>
              </div>
            )
          }
        </div>
